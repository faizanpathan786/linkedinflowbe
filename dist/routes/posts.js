...
function postsRoutes(fastify) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        function publishPostToLinkedIn(userId, postContent) {
            return __awaiter(this, void 0, void 0, function* () {
                var _a;
                const client = yield pool.connect();
                try {
                    const tokenResult = yield client.query(`SELECT access_token, person_urn, expires_at FROM public.linkedin_tokens WHERE user_id = $1`, [userId]);
                    if (tokenResult.rows.length === 0) {
                        const err = new Error('Please connect your LinkedIn account first');
                        err.statusCode = 400;
                    ...
                    if (new Date(tokenData.expires_at) <= new Date()) {
                        const err = new Error('Please reconnect your LinkedIn account');
                        err.statusCode = 401;
                    ...
                    const linkedinResponse = yield linkedinService.createUnifiedPost(tokenData, {
                        ...
                        linkUrl: (_a = postContent.link_url) !== null && _a !== void 0 ? _a : undefined,
                        image: imagePayload,
                    });
                    return (linkedinResponse === null || linkedinResponse === void 0 ? void 0 : linkedinResponse.id) || null;
                }
                finally {
                    client.release();
                }
        ...
        function notifyPublished(userId, postContent, publishedAt) {
            return __awaiter(this, void 0, void 0, function* () {
                try {
                    const client = yield pool.connect();
                    try {
                        const { rows } = yield client.query(`SELECT name, email FROM public."user" WHERE id = $1`, [userId]);
                        if (rows.length === 0)
                            return;
                        const { name, email } = rows[0];
                        yield (0, email_1.sendPostPublishedEmail)(email, name, postContent, publishedAt.toISOString());
        ...
        fastify.post('/posts', {
            ...
            const client = yield pool.connect();
            try {
                let linkedin_post_id = null;
                let status = isScheduled ? 'scheduled' : 'draft';
                if (publish_now) {
                    try {
                        fastify.log.info({
                            has_image_base64: !!image_base64,
                            image_base64_length: image_base64 === null || image_base64 === void 0 ? void 0 : image_base64.length,
                            image_type,
                            post_type,
                        }, 'POST /posts publish payload');
                        linkedin_post_id = yield publishPostToLinkedIn(userId, {
                        ...
                        yield client.query(`INSERT INTO public.posts (user_id, content, post_type, link_url, status)
               VALUES ($1, $2, $3, $4, 'failed')`, [userId, content, post_type, link_url || null]);
                ...
                const result = yield client.query(`INSERT INTO public.posts
           ...
           RETURNING id, user_id, content, post_type, link_url, linkedin_post_id, status, scheduled_at, published_at, image_type, created_at, updated_at`, [
                    ...
                    linkedin_post_id,
                    status,
                    scheduledDate !== null && scheduledDate !== void 0 ? scheduledDate : null,
        ...
        fastify.patch('/posts/:id/publish', (request, reply) => __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            const session = yield auth_1.auth.api.getSession({ headers: request.headers });
            if (!session) {
                return reply.status(401).send({ error: 'Unauthorized' });
            ...
            try {
                ...
                const postResult = yield client.query(`SELECT * FROM public.posts WHERE id = $1 AND user_id = $2`, [id, userId]);
                ...
                try {
                    linkedin_post_id = yield publishPostToLinkedIn(userId, {
                        ...
                        image_base64: (_a = post.image_base64) !== null && _a !== void 0 ? _a : undefined,
                        image_type: (_b = post.image_type) !== null && _b !== void 0 ? _b : undefined,
                    });
                }
                catch (err) {
                    fastify.log.error('LinkedIn publish error on PATCH /publish:', err.message);
                    // Mark as failed in DB
                    yield client.query(`UPDATE public.posts SET status = 'failed', updated_at = NOW() WHERE id = $1`, [id]);
                    return reply.status(err.statusCode || 502).send({
                        success: false,
                        error: err.code || 'LINKEDIN_PUBLISH_FAILED',
                        message: err.message || 'Failed to publish to LinkedIn',
                ...
                const updated = yield client.query(`UPDATE public.posts
           SET status = 'published', linkedin_post_id = $1, published_at = NOW(), updated_at = NOW(),
               image_base64 = NULL, image_type = NULL
           WHERE id = $2
           RETURNING id, user_id, content, post_type, link_url, linkedin_post_id, status, scheduled_at, published_at, created_at, updated_at`, [linkedin_post_id, id]);
        ...
        fastify.get('/posts', (request, reply) => __awaiter(this, void 0, void 0, function* () {
            const session = yield auth_1.auth.api.getSession({ headers: request.headers });
            if (!session) {
                return reply.status(401).send({ error: 'Unauthorized' });
            }
            const client = yield pool.connect();
            try {
                const result = yield client.query(`SELECT id, user_id, content, post_type, link_url, linkedin_post_id, status,
                scheduled_at, published_at, image_type, created_at, updated_at
         FROM public.posts WHERE user_id = $1 ORDER BY created_at DESC`, [session.user.id]);
        ...
        fastify.get('/posts/:id', (request, reply) => __awaiter(this, void 0, void 0, function* () {
            const session = yield auth_1.auth.api.getSession({ headers: request.headers });
            if (!session) {
                return reply.status(401).send({ error: 'Unauthorized' });
            }
            const client = yield pool.connect();
            try {
                const result = yield client.query(`SELECT * FROM public.posts WHERE id = $1 AND user_id = $2`, [request.params.id, session.user.id]);
        ...
        fastify.delete('/posts/:id', (request, reply) => __awaiter(this, void 0, void 0, function* () {
            const session = yield auth_1.auth.api.getSession({ headers: request.headers });
            if (!session) {
                return reply.status(401).send({ error: 'Unauthorized' });
            }
            const client = yield pool.connect();
            try {
                const result = yield client.query(`DELETE FROM public.posts WHERE id = $1 AND user_id = $2 RETURNING id`, [request.params.id, session.user.id]);
                if (result.rows.length === 0) {
                    return reply.status(404).send({ error: 'Post not found' });
                }
